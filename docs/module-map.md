...
The current `crm-kernel` crate is still a practical integrated kernel, but the intended long-term architecture is modular. These module boundaries are explicit so separate agents or teams can take ownership without arguing about scope every time.
...
- `crm-kernel` remains the integrated working kernel
...
1. move module-specific commands and aggregates out of `crm-kernel`
