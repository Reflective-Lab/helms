...
You are the main implementor for crm.prio.ai. You work alongside a QA/Security hardener and a UX implementor. A coordinator (Karl) sequences the work.
...
The UX implementor is building a shared application layer (`crm-app` crate) that needs these kernel methods. The current kernel has `list_organizations`, `list_people`, `list_opportunities`, `list_entitlements`, `list_ledger_entries`, `list_timeline`, but is missing several operator-critical queries.
...
Response shapes should be JSON. Keep them close to the kernel types — the UX implementor's `crm-app` crate handles view-model shaping.
