//! Operator receipt vocabulary for Helm operator-control modules.
//!
//! # Vocabulary
//!
//! This module owns the complete type vocabulary used by Helm's operator-control
//! layer:
//!
//! - [`JobReadinessPacket`] / [`JobReadinessPacketInput`] — read model for "can
//!   this job be trusted enough for an operator to continue reviewing it?"
//! - [`OperatorLedgerEntry`] / [`OperatorLedgerEntryInput`] — deterministic
//!   append-only ledger entries for Helm operator-control receipts.
//! - Supporting enums: [`AdapterReceiptStatus`], [`JobVerdict`],
//!   [`EvidenceReadinessStatus`], [`OperatorLedgerRecordKind`],
//!   [`ReceiptFamily`], [`AuthorityEffect`].
//! - Evidence / fuzzy trace structs: [`JobEvidenceStatus`],
//!   [`FuzzyReadinessTrace`], [`FuzzyMembership`], [`FuzzyRuleActivation`],
//!   [`FuzzyDefuzzifiedScore`].
//! - Free functions: [`job_readiness_packet_payload_hash`],
//!   [`job_readiness_packet_ledger_entry`].
//! - Error type: [`OperatorControlError`].
//!
//! # Non-authority invariant
//!
//! [`OperatorLedgerEntry::new`] always yields [`AuthorityEffect::None`].
//! Helm's operator-control layer is a control-plane journal; it stores ids,
//! refs, and hashes, and it **never** grants domain authority. Calling code that
//! attempts to request domain action authority via
//! [`JobReadinessPacketInput::authorizes_domain_action`] is rejected with
//! [`OperatorControlError::DomainActionAuthorityRequested`].
//!
//! # Why this lives in a neutral contract crate
//!
//! Placing this vocabulary here (RP-LAYERING, RFL-128 → RFL-154) means both
//! `prio-agent-ops` and `workbench-backend` can consume it without introducing a
//! foundation→substrate dependency. The only dependencies are `serde` and
//! `sha2`, so pure consumers never pull in application-kernel, axum, or any
//! platform runtime.
//!
...
/// Input for constructing a [`JobReadinessPacket`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Evidence readiness status for a single clause in a [`JobReadinessPacket`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Fuzzy logic readiness trace attached to a [`JobReadinessPacket`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// A single fuzzy set membership in a [`FuzzyReadinessTrace`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// A single activated fuzzy rule in a [`FuzzyReadinessTrace`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Defuzzified output score in a [`FuzzyReadinessTrace`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Status of the adapter receipt attached to a job readiness packet.
...
/// Job verdict in a [`JobReadinessPacket`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum JobVerdict {
    Satisfied,
...
impl JobVerdict {
    /// Returns the canonical wire string for this verdict.
    pub const fn as_str(self) -> &'static str {
        match self {
...
/// Evidence readiness status for a single clause.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
impl OperatorLedgerEntry {
    /// Builds a deterministic ledger entry. `authority_effect` is always
    /// [`AuthorityEffect::None`] — this crate is a control-plane journal, not
    /// an authority grant.
...
/// Computes the deterministic SHA-256 payload hash for a [`JobReadinessPacket`].
///
/// The hash covers the full packet: its id, all field values, evidence status
/// items, fuzzy trace (if any), forbidden actions, and operator actions. Use
/// this hash as the `payload_hash` field when building an
/// [`OperatorLedgerEntry`] that records this packet.
...
/// Convenience constructor that builds an [`OperatorLedgerEntry`] for a
/// [`JobReadinessPacket`], using [`job_readiness_packet_payload_hash`] as the
/// `payload_hash` and [`OperatorLedgerRecordKind::JobReadinessPacket`] /
/// [`ReceiptFamily::Common`] as the record metadata.
...
/// Input for constructing an [`OperatorLedgerEntry`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Discriminant for the kind of receipt recorded in an [`OperatorLedgerEntry`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
impl OperatorLedgerRecordKind {
    /// Returns the canonical wire string for this record kind.
    pub const fn as_str(self) -> &'static str {
        match self {
...
/// Receipt family classification for an [`OperatorLedgerEntry`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
impl ReceiptFamily {
    /// Returns the canonical wire string for this family.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::Common => "common",
...
/// The authority effect recorded on an [`OperatorLedgerEntry`].
///
/// The only valid value is [`AuthorityEffect::None`]: ledger entries are
/// control-plane journal records and never grant domain authority.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
impl AuthorityEffect {
    /// Returns the canonical wire string for this effect.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::None => "none",
        }
...
/// Errors returned by operator receipt constructors.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum OperatorControlError {
    /// A required string field was empty or whitespace-only.
    EmptyField { field: &'static str },
    /// A backlink id in the backlink list was empty or whitespace-only.
    EmptyBacklink,
    ...
    /// The caller set `authorizes_domain_action = true`; this is forbidden.
...
// ── Private hashing and validation helpers ────────────────────────────────────
