...
| ID | State | Note |
|---|---|---|
| `H-2026-06-15-01` | In progress 2026-06-15 | `helm-module-contracts` and module status APIs landed; serialized verifier-shape tests added; live Quorum evidence feed still needs implementation. |
...
| `H-2026-06-15-05` | In progress 2026-06-15 | Helm now exposes and tests `shell-default`/`live` status serialization; RR manifest/trait integration and transport agreement remain open. |
...
|---|---|---|---|---|---|---|
| `H-2026-06-15-01` | A | Helms + Quorum | Live Helm readiness contract for `helm.operator-control` and `helm.governed-jobs`; default shells must not be presented as live. | `helm-operator-control` and `helm-governed-jobs` expose a live/shell module-state contract or equivalent readiness provider; static/demo fixtures are explicitly labeled; Quorum can feed process receipt, integrity proof, adapter receipt, and Axiom report into the live path; RR strict verifier can reject a shell advertised as live. | RR `D1`; Quorum live readiness feed; `H-2026-06-15-05`. | Quorum operator-control can be marked live only when the verifier sees live module state and live app evidence. |
...
| `H-2026-06-15-05` | B | Helms + RR | Mount vocabulary and live-readiness transport. | RR and Helms settle whether the manifest uses strict `mount_kind` semantics or `mount_kind` plus `integration_status`; the live-readiness transport is documented as provider/feed/packet, not direct domain-state access; verifier behavior is documented. | RR `D1`, RR `D2`; `H-2026-06-15-01`. | `runway.app.json` can express shell vs live Helm mounts without ambiguity. |
