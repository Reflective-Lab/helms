...
| ID | State | Note |
|---|---|---|
| `H-2026-06-15-01` | Done 2026-06-15 (Helm side) | `HelmModuleReadiness::module_state()` and status APIs landed; `helm.operator-control` has an app-owned live readiness feed contract; mounted modules bridge that state into Runway's `HelmModule::module_state()`; Quorum still needs to implement the feed. |
