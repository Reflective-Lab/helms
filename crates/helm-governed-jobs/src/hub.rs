//! Local realtime hub for the governed-jobs module.
//!
//! This is a verbatim copy of `application-server/src/realtime.rs` (the types
//! `RealtimeHub`, `RealtimeEvent`, `RealtimeEventInput`, `RealtimeCursor`,
//! `RealtimeSubscription`).  It exists here because `runway_app_host::EventHubHandle`
//! is too thin for the SSE replay semantics needed by `job_stream.rs`: it has no
//! sequence numbers, no `run_id`/`job_id` fields, and no replay buffer.
//!
//! The full `EventHubHandle` adaptation (merging these semantics upstream into
//! `runway-app-host`) is deferred to a later phase.
