...
mod tests {
    use super::*;
    use crate::admission::default_helms_capabilities;
    use crate::find_truth;
    use crate::intent_compile::compile_intent_for_truth;

    fn intent_for(key: &str) -> IntentPacket {
        let truth = find_truth(key).expect("known truth");
        compile_intent_for_truth(&truth).expect("axiom compiles")
    }

    #[test]
    fn single_shot_excludes_all_alternates() {
        let intent = intent_for("qualify-inbound-lead");
        let opts = AutoRunOptions::default(); // race_alternates: false
        let slate = prepare_candidates(&intent, &default_helms_capabilities(), &opts)
            .expect("selection succeeds");
        assert!(slate.alternate_template_ids.is_empty());
        assert!(
            slate
                .excluded
                .iter()
                .all(|e| e.reason == ExclusionReason::SingleShotRequested)
        );
    }

    #[test]
    fn race_keeps_primary_and_filters_alternates() {
        let intent = intent_for("qualify-inbound-lead");
        let opts = AutoRunOptions {
            race_alternates: true,
            max_candidates: 3,
            relative_cutoff: 0.0, // accept anything that scored
        };
        let slate = prepare_candidates(&intent, &default_helms_capabilities(), &opts)
            .expect("selection succeeds");
    ...
    #[test]
    fn max_candidates_clamps_to_one() {
        let intent = intent_for("qualify-inbound-lead");
        let opts = AutoRunOptions {
            race_alternates: true,
            max_candidates: 0, // would underflow without the clamp
            relative_cutoff: 0.0,
        };
        let slate = prepare_candidates(&intent, &default_helms_capabilities(), &opts)
            .expect("selection succeeds");
        ...
        assert!(
            slate
                .excluded
                .iter()
                .all(|e| e.reason == ExclusionReason::BeyondMaxCandidates)
        );
    }

    #[test]
    fn cutoff_above_one_does_not_eject_primary() {
        let intent = intent_for("qualify-inbound-lead");
        let opts = AutoRunOptions {
            race_alternates: true,
            max_candidates: 3,
            relative_cutoff: 5.0, // pathological, should clamp to 1.0
        };
        let slate = prepare_candidates(&intent, &default_helms_capabilities(), &opts)
            .expect("primary is never filtered out");
        assert!(!slate.primary_template_id.is_empty());
    }

    #[test]
    fn irreversible_intents_refuse_to_race() {
        let mut intent = intent_for("qualify-inbound-lead");
        intent.reversibility = Reversibility::Irreversible;
        let opts = AutoRunOptions {
            race_alternates: true,
            max_candidates: 3,
        ...
        let err = prepare_candidates(&intent, &default_helms_capabilities(), &opts)
            .expect_err("irreversible + race must reject");
        assert!(matches!(err, AutoRunError::IrreversibleCannotRace));
    }

    #[test]
    fn irreversible_intents_can_run_single_shot() {
        let mut intent = intent_for("qualify-inbound-lead");
        intent.reversibility = Reversibility::Irreversible;
        let opts = AutoRunOptions::default(); // race_alternates: false
        let slate = prepare_candidates(&intent, &default_helms_capabilities(), &opts)
            .expect("single-shot is fine for irreversibles");
        assert!(!slate.primary_template_id.is_empty());
    }
}
