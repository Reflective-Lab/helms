...
mod tests {
    ...
    use super::{
        ApprovalFilter, ExecutionState, OperatorApp, OperatorAppError, WorkflowCaseFilter,
    };
    ...
    #[test]
    fn operator_control_preview_requires_injected_live_readiness_feed() {
        let error = app()
            .operator_control_preview()
            .expect_err("static operator-control previews are not synthesized");

        assert!(matches!(
            error,
            OperatorAppError::OperatorControl(message)
                if message.contains("injected live readiness feed")
        ));
    }

    #[test]
    fn operator_control_previews_default_to_empty_without_injected_feed() {
        let previews = app()
