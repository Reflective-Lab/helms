...
use converge_model::{
    TruthCatalog as ConvergeTruthCatalog,
    TruthDefinition as ConvergeTruth,
};
use truth_catalog::{TruthCatalog, TruthDefinition, TruthKey, TruthKind, TruthModuleTouch, to_converge_truth};
...
pub use evaluators::{
    ...
    MatchVisualToTaglineEvaluator,
    MonitorBrandSignalEvaluator,
    ...
    UpgradeSubscriptionPlanEvaluator,
    converge_binding_for_truth,
    converge_truth_definition,
};
pub use overlay::compile_intent_for_truth;
pub use recipes::{organism_binding_for_truth, display_pack_names_for_truth};
...
impl ConvergeTruthCatalog for StaticTruthCatalog {
    fn list_truths(&self) -> Vec<ConvergeTruth> {
        all_truths()
            .into_iter()
            .map(|t| to_converge_truth(t, &CrmPackResolver).unwrap_or_else(|e| panic!("{e}")))
            .collect()
    }
}
...
pub const TRUTHS: &[TruthDefinition] = &[
    ...
    TruthDefinition {
        key: "schedule-strategic-meetings",
        display_name: "Schedule strategic meetings",
        kind: TruthKind::Job,
        summary: "Collapse multi-tool scheduling into a single intent: rank prospects by strategy alignment, resolve availability, and propose a concrete meeting slate with reasoning.",
        feature_path: "truths/jobs/schedule_strategic_meetings.feature",
        actor_roles: &["commercial-operator", "account-owner", "sales-agent"],
        approval_points: &["human confirmation before any meeting is booked"],
        desired_outcomes: &[
            "a ranked meeting slate is proposed with reasoning",
            "each proposed meeting cites strategy alignment evidence",
        ],
        guardrails: &[
            "no meeting shall be auto-booked without human confirmation",
            "candidate ranking must cite pipeline score and strategy context",
        ],
        modules: &[
            TruthModuleTouch {
                module_key: "parties",
                responsibility: "resolve prospect organizations, contacts, and relationship context",
            },
            TruthModuleTouch {
                module_key: "opportunities",
                responsibility: "supply scored pipeline and commercial readiness signals",
            },
            TruthModuleTouch {
                module_key: "conversations",
                responsibility: "provide communication history and scheduling preferences",
            },
            TruthModuleTouch {
                module_key: "tasks",
                responsibility: "create bookable meeting tasks from the proposed slate",
            },
            TruthModuleTouch {
                module_key: "facts",
                responsibility: "record strategy alignment evidence for each candidate",
            },
            TruthModuleTouch {
                module_key: "intents",
                responsibility: "preserve the original free-text scheduling intent",
            },
        ],
        gherkin: include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
    ...
    TruthDefinition {
        ...
        display_name: "Monitor brand signal",
        kind: TruthKind::Job,
        summary: "Ingest external brand mentions, cluster them into narratives, and keep a governed brand-state projection current without auto-publishing anything.",
        feature_path: "truths/jobs/monitor_brand_signal.feature",
        actor_roles: &["brand-manager", "communications-lead", "runtime-agent"],
        approval_points: &[
            "promote SignalIncident at severity high or above",
        ...
        guardrails: &[
            "mentions must retain source URL and retrieval timestamp",
            "clustering must be reproducible from stored embeddings",
            "sentiment must carry calibrated confidence, not a bare label",
            "high-severity incidents cannot auto-promote",
            "no outbound response may be generated by this truth",
        ],
        modules: &[
            TruthModuleTouch {
                module_key: "parties",
                responsibility: "anchor the BrandWatch to an organization, product, or executive",
            },
            TruthModuleTouch {
                module_key: "conversations",
                responsibility: "store raw signal items and cluster membership",
            },
            TruthModuleTouch {
                module_key: "documents",
                responsibility: "store narrative summaries and incident briefs",
            },
            TruthModuleTouch {
                module_key: "memory",
                responsibility: "provide embeddings and semantic retrieval",
            },
            TruthModuleTouch {
                module_key: "workflow",
                responsibility: "run the watch cadence and incident lifecycle",
            },
            TruthModuleTouch {
                module_key: "approvals",
                responsibility: "gate high-severity incident promotion",
            },
            TruthModuleTouch {
                module_key: "intents",
                responsibility: "preserve the operator-facing BrandWatch context",
            },
        ],
        gherkin: include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
    ...
    TruthDefinition {
        ...
        display_name: "Match visual to tagline",
        kind: TruthKind::Job,
        summary: "Match brand-safe visuals with taglines at scale under typed multi-role approval, so campaign pairings ship only with brand, marketing, and storyteller sign-off.",
        feature_path: "truths/jobs/match_visual_to_tagline.feature",
        actor_roles: &[
            "campaign-owner",
            "brand-manager",
            "marketer",
        ...
        approval_points: &[
            ...
            "storyteller approval of narrative and copy",
        ...
        modules: &[
            TruthModuleTouch {
                module_key: "intents",
                responsibility: "carry the CampaignBrief and desired outcome",
            },
            TruthModuleTouch {
                module_key: "documents",
                responsibility: "store briefs, pairings, approvals, and rationale",
            },
            TruthModuleTouch {
                module_key: "memory",
                responsibility: "supply brand guardrails, priors, and multimodal embeddings",
            },
            TruthModuleTouch {
                module_key: "workflow",
                responsibility: "run the multi-gate approval lifecycle",
            },
            TruthModuleTouch {
                module_key: "approvals",
                responsibility: "enforce typed approval gates per role",
            },
            TruthModuleTouch {
                module_key: "parties",
                responsibility: "anchor campaign owner and approving actors",
            },
        ],
        gherkin: include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
    ...
    TruthDefinition {
        key: "evaluate-acquisition-target",
        display_name: "Evaluate acquisition target",
        kind: TruthKind::Job,
        summary: "Converge multi-source evidence into a structured acquisition recommendation with traceable evidence, contradiction surfacing, and honest stopping.",
        feature_path: "truths/jobs/evaluate_acquisition_target.feature",
        actor_roles: &["deal-lead", "investment-committee", "research-analyst"],
        approval_points: &[
            "investment committee approval before recommendation leaves draft",
            "human review when material contradictions are detected",
        ],
        desired_outcomes: &[
            "a recommendation is produced with confidence at least 0.7",
            "all material contradictions are surfaced and documented",
            "each DD dimension cites at least one independent source",
        ],
        guardrails: &[
            "no recommendation without adversarial review passing",
        ...
        modules: &[
            TruthModuleTouch {
                module_key: "facts",
                responsibility: "hold research hypotheses and promoted findings",
            },
            TruthModuleTouch {
                module_key: "documents",
                responsibility: "store source evidence, analysis pages, and the final brief",
            },
            TruthModuleTouch {
                module_key: "workflow",
                responsibility: "coordinate the DD lifecycle and approval gates",
            },
            TruthModuleTouch {
                module_key: "approvals",
                responsibility: "enforce investment committee sign-off",
            },
            TruthModuleTouch {
                module_key: "audit",
                responsibility: "preserve the full evidence and decision trail",
            },
        ],
        gherkin: include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
...
pub const CRM_CATALOG: TruthCatalog<'static> = TruthCatalog::new(TRUTHS);

#[must_use]
pub fn all_truths() -> Vec<TruthDefinition> {
    CRM_CATALOG.all().to_vec()
}

#[must_use]
pub fn truths_by_kind(kind: TruthKind) -> Vec<TruthDefinition> {
    CRM_CATALOG.by_kind(kind).into_iter().copied().collect()
}

#[must_use]
pub fn truths_for_module(module_key: &str) -> Vec<TruthDefinition> {
    CRM_CATALOG.for_module(module_key).into_iter().copied().collect()
}

#[must_use]
pub fn find_truth(key: &str) -> Option<TruthDefinition> {
    TruthKey::parse(key).ok().and_then(|k| CRM_CATALOG.find(&k).copied())
}
...
mod tests {
    ...
    fn starter_catalog_has_eighteen_job_truths() {
        assert_eq!(truths_by_kind(TruthKind::Job).len(), 18);
    }
