...
use organism_runtime::{
    BudgetProbe, CredentialProbe, PackProbe, ReadinessProbe, Registry,
    StructuralResolver, check_readiness,
};
use truth_catalog::{TruthDefinition, TruthOrganismBinding, intent_compile::compile_intent_with_overlay};
use crate::overlay::CrmIntentOverlay;

#[must_use]
pub fn organism_binding_for_truth(truth_key: &str, registry: &Registry) -> Option<TruthOrganismBinding> {
    crate::find_truth(truth_key).and_then(|truth| build_binding(truth, registry))
}

#[must_use]
pub fn display_pack_names_for_truth(truth_key: &str, registry: &Registry) -> Option<Vec<String>> {
    organism_binding_for_truth(truth_key, registry).map(|b| b.pack_names())
}
...
fn build_binding(truth: TruthDefinition, registry: &Registry) -> Option<TruthOrganismBinding> {
    ...
    let intent = compile_intent_with_overlay(&truth, &CrmIntentOverlay)
    ...
    Some(TruthOrganismBinding { truth_key: truth.key, blueprint, binding, readiness })
}

fn binding_recipe(truth: TruthDefinition) -> Option<(Option<&'static str>, IntentBinding, BudgetProbe)> {
    match truth.key {
