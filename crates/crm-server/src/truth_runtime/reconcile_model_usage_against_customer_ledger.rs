...
pub(super) fn execute<S: KernelStore>(
    ...
    engine.register_in_pack(REVENUE_PACK_ID, UsageSummaryAgent { seed: seed.clone() });
    engine.register_in_pack(
        REVENUE_PACK_ID,
        ProviderBillingSummaryAgent { seed: seed.clone() },
    );
    engine.register_in_pack(REVENUE_PACK_ID, LedgerSummaryAgent { seed: seed.clone() });
    engine.register_in_pack(
        REVENUE_PACK_ID,
        EntitlementSummaryAgent { seed: seed.clone() },
    );
    engine.register_in_pack(REVENUE_PACK_ID, ReconciliationMatcherAgent);
    engine.register_in_pack(
        REVENUE_PACK_ID,
        ReconciliationAssessmentAgent { seed: seed.clone() },
    );
    engine.register_in_pack(WORK_PACK_ID, ExceptionRoutingAgent);
...
impl Agent for UsageSummaryAgent {
    fn name(&self) -> &str {
        "UsageSummaryAgent"
    }
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        AgentEffect {
            ...
            proposals: vec![ProposedFact {
                ...
                id: USAGE_SUMMARY_FACT_ID.to_string(),
                content: serde_json::to_string(&UsageSummaryPayload {
                ...
                confidence: 0.99,
                provenance: USAGE_PROVENANCE.to_string(),
            }],
        }
    }
}

impl Agent for ProviderBillingSummaryAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        AgentEffect {
            ...
            proposals: vec![
                ProposedFact {
                    key: ContextKey::Signals,
                    id: PROVIDER_SUMMARY_FACT_ID.to_string(),
                    content: serde_json::to_string(&ProviderSummaryPayload {
                        subscription_id: self.seed.subscription.id,
                    ...
                    confidence: 0.99,
                    provenance: PROVIDER_PROVENANCE.to_string(),
                },
                ProposedFact {
                    key: ContextKey::Signals,
                    id: format!(
...
impl Agent for LedgerSummaryAgent {
    fn name(&self) -> &str {
        "LedgerSummaryAgent"
    }
        ...
        AgentEffect {
            ...
            proposals: vec![
                ProposedFact {
                    ...
                    id: LEDGER_SUMMARY_FACT_ID.to_string(),
                    content: serde_json::to_string(&LedgerSummaryPayload {
                        subscription_id: self.seed.subscription.id,
                    ...
                    confidence: 0.99,
                    provenance: LEDGER_PROVENANCE.to_string(),
                },
                ProposedFact {
                    key: ContextKey::Proposals,
                    id: format!("invoice:reconciliation:{}", self.seed.subscription.id),
                    content: serde_json::json!({
                        "type": "invoice",
                        "state": "open",
                        "subscription_id": self.seed.subscription.id,
...
impl Agent for EntitlementSummaryAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        AgentEffect {
            ...
            proposals: vec![ProposedFact {
                key: ContextKey::Signals,
                id: ENTITLEMENT_SUMMARY_FACT_ID.to_string(),
                content: serde_json::to_string(&EntitlementSummaryPayload {
                    subscription_id: self.seed.subscription.id,
...
impl Agent for ReconciliationAssessmentAgent {
    fn name(&self) -> &str {
        ...
        if matched_provider_to_ledger && max_delta_minor == 0 {
            return AgentEffect {
                ...
                proposals: vec![ProposedFact {
                    ...
                    id: CLEAN_FACT_ID.to_string(),
                    content: serde_json::to_string(&assessment)
                        .expect("clean reconciliation payload should serialize"),
                    confidence: 0.99,
                    provenance: ASSESSMENT_PROVENANCE.to_string(),
                }],
            };
        }
...
impl Agent for ExceptionRoutingAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect {
            ...
            proposals: vec![ProposedFact {
                ...
                id: ROUTE_FACT_ID.to_string(),
                content: serde_json::to_string(&ReconciliationRoutePayload {
                ...
                confidence: 0.98,
                provenance: ROUTING_PROVENANCE.to_string(),
            }],
        }
    }
}
