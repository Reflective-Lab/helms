...
impl Agent for CandidateRankerAgent {
    fn name(&self) -> &str {
        "CandidateRankerAgent"
    }
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        let mut effect = AgentEffect::with_proposal(ProposedFact {
            ...
            id: RANKED_CANDIDATES_FACT_ID.to_string(),
            content,
            confidence: 0.85,
            provenance: RANKING_PROVENANCE.to_string(),
        });

        for candidate in &ranked {
            effect.proposals.push(ProposedFact {
                key: ContextKey::Signals,
                id: format!("meeting:alignment:{}", slug(&candidate.prospect.name)),
                content: serde_json::to_string(&AlignmentEvidence {
                ...
                .unwrap_or_default(),
                confidence: f64::from(candidate.combined_score_bps) / 10_000.0,
                provenance: RANKING_PROVENANCE.to_string(),
            });
        }
...
impl Agent for AvailabilityResolverAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            key: ContextKey::Signals,
            id: AVAILABILITY_FACT_ID.to_string(),
            content,
            confidence: 1.0,
            provenance: SCHEDULING_PROVENANCE.to_string(),
        })
    }
}
...
impl Agent for SlateProposerAgent {
    fn name(&self) -> &str {
        "SlateProposerAgent"
    }
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        let mut effect = AgentEffect::with_proposal(ProposedFact {
            ...
            id: SLATE_FACT_ID.to_string(),
            content: serde_json::to_string(&slate).unwrap_or_default(),
            confidence: 0.9,
            provenance: SCHEDULING_PROVENANCE.to_string(),
        });
        ...
        effect.proposals.push(ProposedFact {
            ...
            id: CONFIRMATION_FACT_ID.to_string(),
...
mod tests {
    ...
    #[test]
    fn schedule_strategic_meetings_executes_end_to_end() {
