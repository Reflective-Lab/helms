...
pub(super) fn execute<S: KernelStore>(
    ...
    engine.register_in_pack(WORK_PACK_ID, RepCapacityAgent { reps: reps.clone() });
    engine.register_in_pack(
        COMMERCIAL_PACK_ID,
        CampaignSolverAgent {
            campaign_name,
...
impl Agent for ProspectPoolAgent {
    fn name(&self) -> &str {
        "ProspectPoolAgent"
    }
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: ROUTING_INPUT_FACT_ID.to_string(),
            content: serde_json::to_string(&routing_input).unwrap_or_default(),
            confidence: 1.0,
            provenance: PLAN_PROVENANCE.to_string(),
        })
    }
}

impl Agent for RepCapacityAgent {
    fn name(&self) -> &str {
        "RepCapacityAgent"
    }
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: CAPACITY_STATUS_FACT_ID.to_string(),
            content: serde_json::to_string(&payload).unwrap_or_default(),
            confidence: 1.0,
            provenance: CAPACITY_PROVENANCE.to_string(),
        })
    }
}

impl Agent for CampaignSolverAgent {
    fn name(&self) -> &str {
        "CampaignSolverAgent"
    }
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: CAMPAIGN_PLAN_FACT_ID.to_string(),
            content: serde_json::to_string(&plan_payload).unwrap_or_default(),
            confidence: solved.plan.confidence,
            provenance: PLAN_PROVENANCE.to_string(),
        })
    }
}

impl Agent for BudgetGuardAgent {
    fn name(&self) -> &str {
        "BudgetGuardAgent"
    }
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: BUDGET_STATUS_FACT_ID.to_string(),
            content: serde_json::to_string(&payload).unwrap_or_default(),
            confidence: 1.0,
            provenance: BUDGET_PROVENANCE.to_string(),
        })
    }
}
