...
impl<S: KernelStore> Agent for ContextGathererAgent<S> {
    fn name(&self) -> &str {
        "ContextGathererAgent"
    }
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: CONTEXT_INDEX_FACT_ID.to_string(),
            content: serde_json::to_string(&RenewalIndexPayload {
                organization_id: self.organization_id,
                entry_count,
            })
            .unwrap_or_default(),
            confidence: 1.0,
            provenance: KNOWLEDGE_PROVENANCE.to_string(),
        })
    }
}

impl Agent for RenewalSignalAgent {
    fn name(&self) -> &str {
        "RenewalSignalAgent"
    }
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        for (signal_id, query) in queries {
            ...
            let results = match block_on_async(async move {
                ...
                Ok(results) => results,
                Err(error) => {
                    effect.facts.push(ConvergeFact::new(
                        ContextKey::Diagnostic,
            ...
            effect.proposals.push(ProposedFact {
                key: ContextKey::Signals,
                id: format!("renewal:signal:{signal_id}"),
                content: serde_json::to_string(&payload).unwrap_or_default(),
                confidence: result.similarity as f64,
                provenance: KNOWLEDGE_PROVENANCE.to_string(),
            });
        }
        effect
...
impl Agent for NegotiationBriefAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: RENEWAL_BRIEF_FACT_ID.to_string(),
            content: serde_json::to_string(&payload).unwrap_or_default(),
            confidence: f64::from(confidence_bps) / 10_000.0,
            provenance: BRIEF_PROVENANCE.to_string(),
        })
    }
}

impl Agent for RenewalTermsAgent {
    fn name(&self) -> &str {
        "RenewalTermsAgent"
    }
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            key: ContextKey::Strategies,
            id: RENEWAL_TERMS_FACT_ID.to_string(),
            content: serde_json::to_string(&payload).unwrap_or_default(),
            confidence: f64::from(payload.confidence_bps) / 10_000.0,
            provenance: TERMS_PROVENANCE.to_string(),
        })
    }
}
...
fn renewal_signals_from_context(ctx: &dyn converge_core::ContextView) -> Vec<RenewalSignalPayload> {
    ctx.get(ContextKey::Signals)
        .iter()
        .filter(|fact| fact.id.starts_with("renewal:signal:"))
