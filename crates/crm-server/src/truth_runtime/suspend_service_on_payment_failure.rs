...
pub(super) fn execute<S: KernelStore>(
    ...
    engine.register_in_pack(REVENUE_PACK_ID, OverdueDetectorAgent);
...
impl Agent for SuspensionDecisionAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, _ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        if should_suspend(&self.seed) {
            return AgentEffect::with_proposal(ProposedFact {
                ...
                id: SUSPENSION_READY_FACT_ID.to_string(),
                content: serde_json::to_string(&SuspensionReadyPayload {
                ...
                confidence: 0.99,
                provenance: SUSPENSION_PROVENANCE.to_string(),
            });
        }

        AgentEffect::with_proposal(ProposedFact {
            key: ContextKey::Strategies,
            id: SUSPENSION_DEFERRED_FACT_ID.to_string(),
            content: serde_json::to_string(&SuspensionDeferredPayload {
                subscription_id: self.seed.subscription.id,
                organization_id: self.seed.organization.id,
...
impl Agent for EntitlementImpactAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            ...
            id: ENTITLEMENT_IMPACT_FACT_ID.to_string(),
            content: serde_json::to_string(&EntitlementImpactPayload {
...
impl Agent for RecoveryPathAgent {
    fn name(&self) -> &str {
    ...
    fn execute(&self, ctx: &dyn converge_core::ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(ProposedFact {
            key: ContextKey::Strategies,
            id: RECOVERY_PATH_FACT_ID.to_string(),
            content: serde_json::to_string(&RecoveryPathPayload {
                summary,
                next_action,
            ...
            confidence: 0.99,
            provenance: RECOVERY_PROVENANCE.to_string(),
        })
    }
}
