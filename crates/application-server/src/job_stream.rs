...
use crate::truth_runtime::{TruthExecutionArtifacts, execute_truth, supports_truth_execution};
...
pub fn job_stream_routes<S>() -> Router<HttpState<S>>
where
    S: KernelStore + Clone + Send + Sync + 'static,
{
    Router::new().route("/v1/jobs/{key}/stream", post(stream_job::<S>))
}
...
async fn stream_job<S>(
    State(state): State<HttpState<S>>,
    axum::Extension(hub): axum::Extension<RealtimeHub>,
    axum::Extension(job_stream_state): axum::Extension<JobStreamState>,
    ...
    S: KernelStore + Clone + Send + Sync + 'static,
{
    ...
    if !supports_truth_execution(&truth_key) {
        return Err((
    ...
    let runtime_stores = state.runtime_stores.clone();
...
async fn run_job_task<S>(
    ...
    hub: RealtimeHub,
    job_stream_state: JobStreamState,
    run_id: String,
    truth_key: String,
    app_id: String,
    inputs: HashMap<String, String>,
    ...
    S: KernelStore + Clone + Send + Sync + 'static,
{
    ...
    let decision_rx = job_stream_state.register_gate_waiter(
        &gate_ref,
        runtime_ref.clone(),
    ...
    let second =
        match execute_truth(&store, &runtime_stores, &truth_key, inputs, actor, false).await {
            Ok(r) => r,
            Err(status) => {
                publish(
                    &hub,
