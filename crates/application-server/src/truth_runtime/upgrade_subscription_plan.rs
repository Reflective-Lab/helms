...
pub(super) async fn execute<S: KernelStore>(
    ...
    engine.register_suggestor_in_pack(REVENUE_PACK_ID, CommercialDeltaAgent { seed: seed.clone() });
...
impl Suggestor for PlanChangeValidationAgent {
    fn name(&self) -> &str {
...
#[async_trait::async_trait]
impl Suggestor for CommercialDeltaAgent {
    fn name(&self) -> &str {
...
mod tests {
    ...
    #[tokio::test]
    async fn upgrade_subscription_plan_executes_end_to_end() {
    ...
    #[tokio::test]
    async fn upgrade_subscription_plan_blocks_for_price_override() {
        let store = InMemoryKernelStore::default_local();
        let runtime_stores = application_storage::AppRuntimeStores {
            context: application_storage::AppContextStore::Memory(
                application_storage::InMemoryContextStore::new(),
