...
impl Suggestor for LeadQualificationAgent {
    fn name(&self) -> &str {
    ...
    async fn execute(&self, ctx: &dyn ContextView) -> AgentEffect {
        ...
        AgentEffect::with_proposal(
            ...
            .with_confidence(QUALIFICATION_CONFIDENCE),
        )
    }
}
...
#[async_trait::async_trait]
impl Suggestor for LeadRoutingAgent {
    fn name(&self) -> &str {
        "prio.lead-routing"
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn qualify_inbound_lead_executes_end_to_end() {
    ...
    #[tokio::test]
    async fn qualify_inbound_lead_disqualifies_non_buyer_signals() {
        for summary in [
    ...
    #[tokio::test]
    async fn qualify_inbound_lead_routes_ambiguous_leads_to_manual_review() {
        let store = InMemoryKernelStore::default_local();
        let inputs = HashMap::from([
    ...
    #[tokio::test]
    async fn qualify_inbound_lead_without_projection_produces_no_side_effects() {
        let store = InMemoryKernelStore::default_local();
        let inputs = HashMap::from([
    ...
    #[tokio::test]
    async fn qualify_inbound_lead_idempotent_organization_upsert() {
        let store = InMemoryKernelStore::default_local();
    ...
    #[tokio::test]
    async fn qualify_inbound_lead_sequential_runs_share_store_state() {
        let store = InMemoryKernelStore::default_local();
...
#[async_trait::async_trait]
    ...
    fn build_engine_and_context(
        ...
        Engine,
        Context,
        converge_kernel::TypesRootIntent,
        TypesRunHooks,
    ) {
    ...
    #[tokio::test]
    async fn silent_agent_converges_without_meeting_criteria() {
        let inputs = standard_inputs();
        let (mut engine, context, intent, hooks) = build_engine_and_context(&inputs);
        engine.register_suggestor_in_pack(COMMERCIAL_PACK_ID, SilentAgent);
        engine.register_suggestor_in_pack(WORK_PACK_ID, LeadRoutingAgent);
        let result = engine
    ...
    #[tokio::test]
    async fn malformed_payload_converges_but_projection_fails_gracefully() {
        let inputs = standard_inputs();
        let (mut engine, context, intent, hooks) = build_engine_and_context(&inputs);
        engine.register_suggestor_in_pack(COMMERCIAL_PACK_ID, MalformedPayloadAgent);
        engine.register_suggestor_in_pack(WORK_PACK_ID, LeadRoutingAgent);
        let result = engine
            .run_with_types_intent_and_hooks(context, &intent, hooks)
            .await
    ...
    #[tokio::test]
    async fn noisy_agent_extra_facts_do_not_affect_criteria() {
        let inputs = standard_inputs();
        let (mut engine, context, intent, hooks) = build_engine_and_context(&inputs);
        engine.register_suggestor_in_pack(COMMERCIAL_PACK_ID, NoisyQualificationAgent);
        engine.register_suggestor_in_pack(WORK_PACK_ID, LeadRoutingAgent);
        let result = engine
            .run_with_types_intent_and_hooks(context, &intent, hooks)
            .await
    ...
    #[tokio::test]
    async fn low_confidence_agent_still_converges() {
        let inputs = standard_inputs();
        let (mut engine, context, intent, hooks) = build_engine_and_context(&inputs);
        engine.register_suggestor_in_pack(COMMERCIAL_PACK_ID, LowConfidenceAgent);
        engine.register_suggestor_in_pack(WORK_PACK_ID, LeadRoutingAgent);
        let result = engine
            .run_with_types_intent_and_hooks(context, &intent, hooks)
            .await
    ...
    fn run_qualify_blocking(
        ...
        tokio::runtime::Builder::new_current_thread()
            ...
            .block_on(run_qualify_async(store, inputs, persist))
    }

    #[tokio::test]
    async fn blocked_truth_retried_without_new_evidence_is_idempotent() {
        let store = InMemoryKernelStore::default_local();
        let org_id = uuid::Uuid::new_v4();
        ...
        let first = run_qualify_async(&store, ambiguous_inputs.clone(), true)
            .await
            .expect("first execution should succeed");
        ...
        let second = run_qualify_async(&store, ambiguous_inputs, true)
            .await
            .expect("second execution should succeed");
    ...
    mod security_properties {
        ...
        proptest! {
            ...
            fn sql_injection_in_org_name_is_safely_stored(
                ...
                let result = run_qualify_blocking(&store, inputs, true);
