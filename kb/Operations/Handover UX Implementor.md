...
You are the UX implementor for Outcome Workbench. You work alongside a main implementor (backend/kernel) and a QA/Security hardener. A coordinator (Karl) sequences the work.

## Context

Outcome Workbench is a JTBD-driven application layer built as a Converge application. The backend has 9 executable truths, 20 capability modules, and a full domain model — but everything is in-memory, there are limited query surfaces, and the desktop app is a static shell.
...
Read [[Coordinator Handoff]] for full architectural context. Key principle: **this is an operator-first, job-centric interface — not a traditional record-navigation CRM.**
...
```
Svelte component → Tauri command → workbench-backend crate → KernelStore
Ratatui widget   → workbench-backend crate → KernelStore (direct, no IPC)
```

The `workbench-backend` crate is transport-agnostic. No axum, no tonic, no HTTP types. It takes a `KernelStore` and exposes operator-oriented use cases.
...
[dependencies]
application-kernel = { path = "../crm-kernel" }
application-storage = { path = "../crm-storage" }
...
- Do not call the HTTP server from Svelte — go through Tauri commands → workbench-backend
...
- `cargo build -p workbench-backend` compiles
